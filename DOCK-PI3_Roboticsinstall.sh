...
# Compilar FFmpeg con soporte mmal (decodificacion de video acelerada por hardware)
sudo apt-get -y install libavutil-dev libavcodec-dev libavformat-dev libavfilter-dev libswscale-dev libavresample-dev
...
cd /home/pi/develop
git clone --depth 1 https://github.com/mickelson/attract attract
cd attract && cp -R config/* /home/pi/.attract/
make -j4 USE_GLES=1
...
cd
#### config ######
cat > /home/pi/.attract/attract.cfg <<_EOF_
...
general	
	language             es	
	exit_command         sudo poweroff	
	exit_message         Apagar sistema	
	...
	menu_layout          Attrac-Man	

layout_config	Attrac-Man
	param                enable_Lmarquee No	
	param                enable_bg blue	
	param                enable_bloom No	
	param                enable_cab moon	
